...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ error?: string; message?: string }> }) {
  ...
  const { data: history } = player ? await supabase.from("profile_change_history").select("id, changed_at").eq("player_id", player.id).order("changed_at", { ascending: false }).limit(10) : { data: [] };
  ...
  return <main className="container-page py-10"><PortalBackLink /><h1 className="text-3xl font-black text-ink">Privacy and account settings</h1>
    ...
    <section className="mt-6 rounded-3xl border border-line bg-white p-6"><h2 className="text-xl font-black">Account security</h2><p className="mb-4 mt-2 text-sm text-muted">Add an authenticator-app sign-in factor.</p><MfaSettings /></section>
    <section className="mt-6 rounded-3xl border border-line bg-white p-6"><h2 className="text-xl font-black">Profile change history</h2><p className="mt-2 text-sm text-muted">The latest profile updates are recorded for accountability.</p><ul className="mt-4 space-y-2">{history?.map((entry) => <li key={entry.id} className="rounded-xl bg-slate-50 px-4 py-3 text-sm text-muted">Profile updated {new Date(entry.changed_at).toLocaleString()}</li>)}</ul>{!history?.length ? <p className="mt-4 text-sm text-muted">No changes recorded yet.</p> : null}</section>
